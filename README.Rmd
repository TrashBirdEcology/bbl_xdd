...
We can use various search terms to identify potential uses of the BBL data or results in the literature. For example, here we found `r bbl_xdd %>% dplyr::select(-filename, -searchterm, -highlight) %>% n_distinct()` unique publications which used any of the following phrases exactly: 

Exacts earch phrase | Number of unique records in xDD/GDD 
--------------|----
"North American Bird Banding Laboratory" | `r bbl_xdd %>% filter(searchterm=="North American Bird Banding Laboratory") %>% dplyr::select(-filename, -highlight) %>% n_distinct()` 
"BBL" |  `r bbl_xdd %>% filter(searchterm=="BBL") %>% dplyr::select(-filename, -highlight) %>% n_distinct()` 
"Bird Banding Laboratory" | `r bbl_xdd %>% filter(searchterm=="Bird Banding Laboratory") %>% dplyr::select(-filename, -highlight) %>% n_distinct()` 


The large number of records containing "BBL" are due to some gas and oil acronym. 
```{r pubsvtime, echo=FALSE}
tmp <- bbl_xdd %>% distinct(coveryear, pubname, publisher, title, authors, searchterm)   
...
Removing the records with terms The large number of records containing "BBL" are due to some gas and oil acronym. 
```{r pubsvtime, echo=FALSE}
temp <- bbl_xdd %>% 
  
str_detect(bbl_xdd$highlight, "oil")
           
tmp <- bbl_xdd %>% distinct(coveryear, pubname, publisher, title, authors, searchterm, highlights)   
ggplot(tmp %>% group_by(searchterm, coveryear) %>% 
         summarise(total_pubs = n()) %>% na.omit() %>% 
         filter(coveryear <= 2022, coveryear >=1980)) +
  geom_line(aes(x=coveryear, y=total_pubs, color=searchterm))+
...
When we remove the redundancies in the records returned using the abovementioned searches,`r bbl_xdd %>% dplyr::select(-filename, -highlight, -searchterm) %>% n_distinct() + bbl_xdd %>% dplyr::select(-filename, -highlight, -searchterm) %>% filter(is.na(doi))%>% distinct()  %>% nrow() ` remain. 
...
tmp <-  bbl_xdd %>% dplyr::select(-filename, -searchterm) %>% distinct()
tmp <- bind_rows(tmp, bbl_xdd %>% dplyr::select(-filename, -searchterm)%>% filter(is.na(doi)) %>% distinct())
...
tmp <- bbl_xdd %>% distinct(coveryear, pubname, publisher, title, authors, searchterm)%>% group_by(pubname) %>% 
